...
-- perform quries on JOIN,SUBQUERY,FUNCTIONS(built_in) and USER DEFINE
-- JOINS 
-- join 2  table with inner inner join tables students_inernship and students_scolarship
-- 1.inner join
select student_internships.student_id,student_internships.company_name 
from student_internships
inner join student_scholarships on student_internships.student_id=student_scholarships.student_id;

-- 2.left join
-- join exam_results and exams table with use of left join quarie
select exam_results.exam_id ,exam_results.marks_obtained
from exam_results
left join exams on exam_results.exam_id=exams.exam_id;

-- 3.right join
-- join course and department table with right join qurei
select courses.department_id,courses.course_name
from courses
right join departments on courses.department_id=departments.department_id;

-- 4.self join
-- use self join operation on proffesors table
select p.professor_id,d.department_id
from professors p
inner join professors d
on p.department_id=d.professor_id;
